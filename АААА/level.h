...
{
	tinyxml2::XMLDocument levelFile; //Загружаем файл 
// Загружаем XML-карту
	...
	// Работа со слоями
	tinyxml2::XMLElement* groupElement;
	groupElement = map->FirstChildElement("group");
	tinyxml2::XMLElement* layerElement;
	layerElement = groupElement->FirstChildElement("layer");
	while (layerElement)
	{
		Layer layer;
	...
	if (groupElement->FirstChildElement("objectgroup") != NULL)
	{
		objectGroupElement = groupElement->FirstChildElement("objectgroup");
